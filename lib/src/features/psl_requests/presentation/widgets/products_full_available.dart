...
class ProductsFullAvailable extends StatefulWidget {
  ...
  @override
  State<ProductsFullAvailable> createState() => _ProductsFullAvailableState();
}

class _ProductsFullAvailableState extends State<ProductsFullAvailable> {
  ...
  void initState() {
    ...
    for (VerificationItem verificationItem
      ...
      for (var element in verificationItem.bloodBanks!) {
        if (!hospitals.contains(element["hospital"]["name"])) {
          hospitals.add(element["hospital"]["name"]);
        }
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const Text(
          "Les produits demandés ont été trouvés et réservés pour vous. Veuillez effectuer le paiement dans les 10 minutes pour confirmer votre demande.",
        ),
